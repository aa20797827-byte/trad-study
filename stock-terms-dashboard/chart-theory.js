...
window.showChart = function(){
  ...
  setTimeout(async function(){
    try {
      var mkt=await fetchMarket();
      var el=document.getElementById('ct-market-card');
      if(el) el.innerHTML=buildMarketCard(mkt)||'';
    } catch(_){}
  }, 400);
  // 알림 폴링 재개 (저장된 알림 있으면)
  setTimeout(function(){ if(_alertGetList().filter(function(a){return a.active;}).length) _ctStartAlertPolling(); }, 500);
  // 기본 종목 자동 분석 실행
  setTimeout(function(){
...
function _ctRenderStats(hist){
  ...
  var pnls=closed.map(function(h){return (h.exitPrice-h.price)/h.price*100;});
  var avgPnl=pnls.length?Math.round(pnls.reduce(function(a,b){return a+b;},0)/pnls.length*10)/10:null;
  var bestPnl=pnls.length?Math.round(Math.max.apply(null,pnls)*10)/10:null;
  var worstPnl=pnls.length?Math.round(Math.min.apply(null,pnls)*10)/10:null;

  el.innerHTML='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:8px">'
    ...
    +'<div style="font-size:10px;color:#6b7280;margin-bottom:4px">결과 승률</div>'
    +'<div style="font-size:20px;font-weight:900;color:'+(wr>=55?'#22c55e':wr>=40?'#f59e0b':'#ef4444')+'">'+wr+'%</div></div>':'<div></div>')
    ...
    +'</div>'
    +(avgPnl!==null?'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px">'
    +'<div style="padding:10px;background:var(--s2);border-radius:10px;text-align:center">'
    +'<div style="font-size:10px;color:#6b7280;margin-bottom:4px">평균 실현 손익</div>'
    +'<div style="font-size:18px;font-weight:900;color:'+(avgPnl>=0?'#22c55e':'#ef4444')+'">'+(avgPnl>=0?'+':'')+avgPnl+'%</div></div>'
    +'<div style="padding:10px;background:rgba(34,197,94,.06);border-radius:10px;text-align:center">'
    +'<div style="font-size:10px;color:#22c55e;margin-bottom:4px">최고 수익</div>'
    +'<div style="font-size:18px;font-weight:900;color:#22c55e">+'+(bestPnl||0)+'%</div></div>'
    +'<div style="padding:10px;background:rgba(239,68,68,.06);border-radius:10px;text-align:center">'
    +'<div style="font-size:10px;color:#ef4444;margin-bottom:4px">최대 손실</div>'
    +'<div style="font-size:18px;font-weight:900;color:#ef4444">'+(worstPnl||0)+'%</div></div>'
    +'</div>':'');
}
...
function renderHistory(){
  ...
  if(jaEl) jaEl.innerHTML=buildMonthlyReview(hist)+buildJournalAnalysis(hist);
  // 실현 손익 통계 계산
  var closed=hist.filter(function(h){return h.exitPrice&&h.price;});
  var totalPnl=closed.reduce(function(acc,h){
    var pnl=(h.exitPrice-h.price)/h.price*100;
    return acc+pnl;
  },0);
  var avgPnl=closed.length?Math.round(totalPnl/closed.length*10)/10:null;

  el.innerHTML = hist.map(function(h){
    var rC = h.result==='성공'?'#22c55e':h.result==='실패'?'#ef4444':h.result==='보유중'?'#60a5fa':'#6b7280';
    var pnlPct = h.exitPrice&&h.price ? Math.round((h.exitPrice-h.price)/h.price*1000)/10 : null;
    var pnlC   = pnlPct===null?'#6b7280':pnlPct>=0?'#22c55e':'#ef4444';
    return '<tr>'
      +'<td style="font-size:11px">'+h.date+'<br><span style="color:#4b5563">'+h.time+'</span></td>'
      ...
      +'<td>'
        +(h.exitPrice
          ? '<input type="number" value="'+h.exitPrice+'" onchange="window._ctSetExit('+h.id+',this.value)" '
            +'style="width:80px;background:transparent;border:1px solid var(--bd);border-radius:4px;color:var(--tx);font-size:11px;padding:2px 4px">'
          : '<input type="number" placeholder="청산가" onchange="window._ctSetExit('+h.id+',this.value)" '
            +'style="width:80px;background:transparent;border:1px solid var(--bd);border-radius:4px;color:#6b7280;font-size:11px;padding:2px 4px">')
      +'</td>'
      +'<td style="font-weight:800;color:'+pnlC+'">'+(pnlPct!==null?(pnlPct>=0?'+':'')+pnlPct+'%':'—')+'</td>'
      ...
      +'</td>'
    +'</tr>'
    +(h.memo?'<tr><td colspan="8" style="padding:4px 12px 8px;font-size:11px;color:#f59e0b;background:rgba(245,158,11,.04)">📝 '+h.memo+'</td></tr>':'');
  }).join('')
  +(avgPnl!==null?'<tr><td colspan="8" style="padding:8px 12px;font-size:12px;color:var(--mt);border-top:1px solid var(--bd)">실현 거래 평균 손익: <b style="color:'+(avgPnl>=0?'#22c55e':'#ef4444')+'">'+(avgPnl>=0?'+':'')+avgPnl+'%</b> ('+closed.length+'건)</td></tr>':'');
}

// 청산가 저장
window._ctSetExit = function(id, val){
  var v=parseFloat(val); if(!v||v<=0) return;
  var hist=JSON.parse(localStorage.getItem('ct_hist')||'[]');
  var rec=hist.find(function(h){return h.id===id;}); if(!rec) return;
  rec.exitPrice=v;
  if(!rec.result) rec.result = v>=rec.price?'성공':'실패';
  localStorage.setItem('ct_hist',JSON.stringify(hist));
...
window._ctSwitchTab = function(id){
  ...
  if(id==='watchlist'){ setTimeout(window._ctRefreshWatchlist, 100); }
};
...
function _showAnalysis(out, ticker, closes, opens, highs, lows, vols, curPrice, currency, name, source){
  try {
    ...
    // 매크로 신호 + 공황/탐욕 지수
    var macroHtml = '', fgHtml = '', miniChartHtml = '', w52Html = '';
    ...
    // 52주 고저가 위치
    try {
      var w52hi=highs.length?Math.max.apply(null,highs):0;
      var w52lo=lows.length?Math.min.apply(null,lows):0;
      if(w52hi>w52lo&&curPrice){
        var fp4=function(v){ return formatPrice(v, currency); };
        var pos52=Math.round((curPrice-w52lo)/(w52hi-w52lo)*100);
        var pos52C=pos52>=80?'#ef4444':pos52>=50?'#f59e0b':'#22c55e';
        w52Html='<div style="margin-bottom:10px;padding:10px 14px;background:var(--bg);border-radius:10px;border:1px solid var(--bd)">'
          +'<div style="display:flex;justify-content:space-between;font-size:10px;color:#6b7280;margin-bottom:4px">'
          +'<span>📉 52주 저점 '+fp4(Math.round(w52lo))+'</span>'
          +'<span style="color:'+pos52C+';font-weight:800">현재 '+pos52+'% 위치</span>'
          +'<span>📈 52주 고점 '+fp4(Math.round(w52hi))+'</span>'
          +'</div>'
          +'<div style="height:8px;background:var(--s2);border-radius:4px;overflow:hidden;position:relative">'
          +'<div style="height:100%;width:'+pos52+'%;background:linear-gradient(to right,#22c55e,'+pos52C+');border-radius:4px"></div>'
          +'<div style="position:absolute;left:'+pos52+'%;top:-2px;width:3px;height:12px;background:#fff;border-radius:2px;transform:translateX(-50%)"></div>'
          +'</div></div>';
      }
    } catch(_){}
    ...
    // 펀더멘털 + 뉴스 병렬 fetch
    Promise.all([
      fetchFundamental(ticker).catch(function(){ return null; }),
      fetchNews(ticker).catch(function(){ return []; })
    ]).then(function(results){
      var fund=results[0], news=results[1];
      var fel=document.getElementById('ct-fundamental-area');
      if(fel) fel.innerHTML = buildFundamentalCard(fund, currency);
      var nel=document.getElementById('ct-news-area');
      if(nel) nel.innerHTML = buildNewsCard(news, ticker);
    });
  } catch(e){
  ...
  if(tvData){
    try {
      ...
      Promise.all([
        ...
        var fel=document.getElementById('ct-fundamental-area');
        if(fel) fel.innerHTML=buildFundamentalCard(res[0], tvData.currency||'USD');
        var nel=document.getElementById('ct-news-area');
        if(nel) nel.innerHTML=buildNewsCard(res[1], ticker);
      });
      return;
...
// ══════════════════════════════════════
// ── 퀀트 통계 (Sharpe / MDD / VaR / 변동성) ──
// ══════════════════════════════════════
function computeQuantStats(closes){
  ...
  var annRet=mean*252*100;

  // 샤프 (무위험 수익률 3%)
  var sharpe=annVol>0?(annRet/100-0.03)/(annVol/100):0;
  ...
  // VaR (역사적 시뮬레이션)
  var sorted=[].concat(rets).sort(function(a,b){return a-b;});
  var var95=sorted[Math.floor(sorted.length*0.05)];
  var var99=sorted[Math.floor(sorted.length*0.01)];

  // 수익률 구간별
  var ret1m=n>=21?  (closes[n-1]-closes[n-22])/closes[n-22]*100:null;
  var ret3m=n>=63?  (closes[n-1]-closes[n-64])/closes[n-64]*100:null;
  var ret6m=n>=126? (closes[n-1]-closes[n-127])/closes[n-127]*100:null;

  // 상승일 비율
  var upDays=rets.filter(function(r){return r>0;}).length;
  var winRate=Math.round(upDays/rets.length*100);

  return {
    annVol:   Math.round(annVol*10)/10,
    annRet:   Math.round(annRet*10)/10,
    sharpe:   Math.round(sharpe*100)/100,
    mdd:      Math.round(mdd*1000)/10,
    var95:    Math.round(Math.abs(var95)*1000)/10,
    var99:    Math.round(Math.abs(var99)*1000)/10,
    ret1m:    ret1m!==null?Math.round(ret1m*10)/10:null,
    ret3m:    ret3m!==null?Math.round(ret3m*10)/10:null,
    ret6m:    ret6m!==null?Math.round(ret6m*10)/10:null,
    winRate:  winRate,
    days:     n
  };
}
...
// ── 퀀트 카드 렌더 ──
function buildQuantCard(qs, fp){
  if(!qs) return '';
  function retC(v){ return v===null?'#6b7280':v>=0?'#22c55e':'#ef4444'; }
  function retS(v){ return v===null?'—':(v>=0?'+':'')+v+'%'; }
  var sharpeC=qs.sharpe>=1?'#22c55e':qs.sharpe>=0?'#f59e0b':'#ef4444';
  var sharpeL=qs.sharpe>=2?'우수':qs.sharpe>=1?'양호':qs.sharpe>=0?'보통':'불량';
  return '<div style="margin:14px 0;background:var(--bg);border:1px solid var(--bd);border-radius:14px;overflow:hidden">'
  +'<div style="padding:11px 16px;background:var(--s1);border-bottom:1px solid var(--bd);font-size:14px;font-weight:800;color:var(--tx)">📐 퀀트 통계 ('+qs.days+'일 기준)</div>'
  +'<div style="padding:14px 16px">'
  // 수익률 행
  +'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:10px">'
  +['1개월','3개월','6개월'].map(function(label,i){
    var v=[qs.ret1m,qs.ret3m,qs.ret6m][i];
    return '<div style="padding:8px;background:var(--s2);border-radius:8px;text-align:center">'
      +'<div style="font-size:10px;color:#6b7280">'+label+' 수익률</div>'
      +'<div style="font-size:16px;font-weight:900;color:'+retC(v)+'">'+retS(v)+'</div></div>';
  }).join('')
  +'</div>'
  // 핵심 지표
  +'<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:10px">'
  +'<div style="padding:10px;background:var(--s2);border-radius:8px">'
  +'<div style="font-size:10px;color:#6b7280;margin-bottom:3px">샤프 지수 <span style="color:#4b5563">(1↑ 양호)</span></div>'
  +'<div style="font-size:20px;font-weight:900;color:'+sharpeC+'">'+qs.sharpe+'</div>'
  +'<div style="font-size:10px;color:'+sharpeC+'">'+sharpeL+'</div></div>'
  +'<div style="padding:10px;background:rgba(239,68,68,.08);border-radius:8px;border:1px solid rgba(239,68,68,.15)">'
  +'<div style="font-size:10px;color:#ef4444;margin-bottom:3px">MDD (최대 낙폭)</div>'
  +'<div style="font-size:20px;font-weight:900;color:#ef4444">-'+qs.mdd+'%</div>'
  +'<div style="font-size:10px;color:#6b7280">분석 기간 내</div></div>'
  +'</div>'
  +'<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px">'
  +'<div style="padding:8px;background:var(--s2);border-radius:8px;text-align:center">'
  +'<div style="font-size:10px;color:#6b7280">연환산 변동성</div>'
  +'<div style="font-size:13px;font-weight:800;color:#f59e0b">'+qs.annVol+'%</div></div>'
  +'<div style="padding:8px;background:rgba(239,68,68,.06);border-radius:8px;text-align:center">'
  +'<div style="font-size:10px;color:#6b7280">VaR 95%</div>'
  +'<div style="font-size:13px;font-weight:800;color:#ef4444">-'+qs.var95+'%</div></div>'
  +'<div style="padding:8px;background:rgba(239,68,68,.06);border-radius:8px;text-align:center">'
  +'<div style="font-size:10px;color:#6b7280">VaR 99%</div>'
  +'<div style="font-size:13px;font-weight:800;color:#ef4444">-'+qs.var99+'%</div></div>'
  +'<div style="padding:8px;background:rgba(34,197,94,.06);border-radius:8px;text-align:center">'
  +'<div style="font-size:10px;color:#6b7280">상승일 비율</div>'
  +'<div style="font-size:13px;font-weight:800;color:#22c55e">'+qs.winRate+'%</div></div>'
  +'</div>'
  +'<div style="margin-top:8px;font-size:10px;color:#4b5563">⚠ VaR: 1일 기준, 역사적 시뮬레이션. 샤프: 무위험 수익률 3% 가정. 과거 데이터 기반이며 미래를 보장하지 않습니다.</div>'
  +'</div></div>';
}
...
async function fetchFundamental(ticker){
  try {
    ...
    var resp=await fetch('/api/fundamental?symbol='+encodeURIComponent(ticker),{signal:ctrl.signal});
    if(!resp||!resp.ok) return null;
    var d=await resp.json();
...
// ── 펀더멘털 카드 렌더 ──
function buildFundamentalCard(f, cur){
  if(!f) return '';
  var fp3=function(v,unit){
    if(v===null||v===undefined) return '—';
    if(unit==='%') return (v*100).toFixed(1)+'%';
    if(unit==='x') return v.toFixed(1)+'x';
    if(unit==='cap'){
      if(cur==='KRW'){ if(v>=1e12) return (v/1e12).toFixed(1)+'조'; if(v>=1e8) return (v/1e8).toFixed(0)+'억'; return v.toLocaleString(); }
  ...
  var recMap={strongBuy:'강력 매수',buy:'매수',hold:'보유',sell:'매도',strongSell:'강력 매도'};
  var recC={strongBuy:'#22c55e',buy:'#4ade80',hold:'#f59e0b',sell:'#f97316',strongSell:'#ef4444'};
  var rec=f.recommendationKey;

  var rows=[
    {label:'PER (주가수익비율)',    val:fp3(f.per,'x'),      tip:'10x 이하 저평가 / 20x+ 고평가 (업종 따라 다름)'},
    {label:'Forward PER',          val:fp3(f.forwardPer,'x'),tip:'향후 예상 이익 기준 PER'},
    {label:'PBR (주가순자산비율)',  val:fp3(f.pbr,'x'),      tip:'1x 이하 = 청산가치 이하'},
    {label:'EPS (주당순이익)',      val:f.eps!==null&&f.eps!==undefined?(cur==='KRW'?Math.round(f.eps).toLocaleString()+'원':'$'+f.eps.toFixed(2)):'—', tip:''},
    {label:'ROE (자기자본이익률)',  val:fp3(f.roe,'%'),       tip:'15%+ 우량 / 음수면 적자'},
    {label:'ROA (총자산이익률)',    val:fp3(f.roa,'%'),       tip:'5%+ 양호'},
    {label:'부채/자기자본',         val:fp3(f.debtToEquity,'x'),tip:'1x 이하 안전 / 높을수록 레버리지 위험'},
    {label:'유동비율',              val:fp3(f.currentRatio,'x'),tip:'1.5x+ 단기 상환 안정'},
    {label:'영업이익률',            val:fp3(f.operatingMargins,'%'),tip:''},
    {label:'순이익률',              val:fp3(f.profitMargins,'%'),tip:''},
    {label:'매출 성장률',           val:fp3(f.revenueGrowth,'%'),tip:''},
    {label:'배당수익률',            val:fp3(f.dividendYield,'%'),tip:''},
    {label:'베타 (시장 민감도)',    val:f.beta!==null&&f.beta!==undefined?f.beta.toFixed(2)+'x':'—',tip:'1.0=시장 동일 / 1.5=1.5배 변동성'},
    {label:'52주 고/저',            val:(f.week52High&&f.week52Low)?(cur==='USD'?'$'+f.week52Low.toFixed(2)+' ~ $'+f.week52High.toFixed(2):f.week52Low.toLocaleString()+'~'+f.week52High.toLocaleString()):'—',tip:''},
    {label:'시가총액',              val:fp3(f.marketCap,'cap'),tip:''},
    {label:'애널리스트 목표가',     val:(cur==='USD'&&f.targetMeanPrice)?'$'+f.targetMeanPrice.toFixed(2):(f.targetMeanPrice?f.targetMeanPrice.toLocaleString()+'원':'—'),tip:(f.numberOfAnalystOpinions?f.numberOfAnalystOpinions+'명 컨센서스':'')},
  ];

  return '<div style="margin:14px 0;background:var(--bg);border:1px solid var(--bd);border-radius:14px;overflow:hidden">'
  +'<div style="padding:11px 16px;background:var(--s1);border-bottom:1px solid var(--bd);display:flex;align-items:center;justify-content:space-between">'
  +'<div style="font-size:14px;font-weight:800;color:var(--tx)">📋 펀더멘털 분석</div>'
  +(rec?'<div style="padding:4px 12px;border-radius:20px;background:'+(recC[rec]||'#6b7280')+'22;border:1px solid '+(recC[rec]||'#6b7280')+';font-size:12px;font-weight:800;color:'+(recC[rec]||'#6b7280')+'">'+(recMap[rec]||rec)+'</div>':'')
  +'</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:0">'
  +rows.filter(function(r){return r.val!=='—';}).map(function(r,i){
    var bg=i%4<2?'transparent':'rgba(255,255,255,.015)';
    return '<div style="padding:9px 14px;border-bottom:1px solid rgba(255,255,255,.04);background:'+bg+'">'
      +'<div style="font-size:10px;color:#6b7280">'+r.label+(r.tip?' <span title="'+r.tip+'">ℹ</span>':'')+'</div>'
      +'<div style="font-size:13px;font-weight:800;color:var(--tx)">'+r.val+'</div>'
      +'</div>';
  }).join('')
  +'</div></div>';
}
...
// ── 시장 현황 대시보드 ──
// ══════════════════════════════════════
var _mktCache = null, _mktTs = 0;

async function fetchMarket(){
  if(_mktCache && Date.now()-_mktTs < 180000) return _mktCache;
  try {
    var ctrl=new AbortController(); setTimeout(function(){ctrl.abort();},12000);
    var resp=await fetch('/api/market',{signal:ctrl.signal});
    if(!resp||!resp.ok) return null;
    var d=await resp.json();
    _mktCache=d; _mktTs=Date.now();
    return d;
  } catch(e){ return null; }
}

function buildMarketCard(mkt){
  if(!mkt||!mkt.items) return '';
  var catOrder={index:0,fear:1,commodity:2,crypto:3,fx:4};
  var items=mkt.items.filter(function(i){return i.price!==null;});
  if(!items.length) return '';

  var catLabel={index:'주요 지수',fear:'공포/탐욕',commodity:'원자재',crypto:'암호화폐',fx:'환율'};
  var groups={};
  ...
  function chip(item){
    var c=item.chg>=0.5?'#22c55e':item.chg<=-0.5?'#ef4444':'#9ca3af';
    var arrow=item.chg>=0.5?'▲':item.chg<=-0.5?'▼':'—';
    var pStr=item.label==='원/달러'?Math.round(item.price).toLocaleString()+'원':
             item.label==='달러지수'?item.price.toFixed(2):
             item.cat==='crypto'?'$'+Math.round(item.price).toLocaleString():
             item.price>=100?item.price.toFixed(1):item.price.toFixed(2);
    return '<div style="padding:7px 11px;background:var(--s2);border-radius:9px;border:1px solid rgba(255,255,255,.06);min-width:80px">'
      +'<div style="font-size:10px;color:#6b7280;margin-bottom:2px">'+item.label+'</div>'
      +'<div style="font-size:13px;font-weight:800;color:var(--tx)">'+pStr+'</div>'
      +'<div style="font-size:11px;font-weight:700;color:'+c+'">'+arrow+' '+(item.chg!==null?(item.chg>=0?'+':'')+item.chg+'%':'—')+'</div>'
      +'</div>';
  }

  var ts=new Date(_mktTs).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'});
  var html='<div style="margin-bottom:12px;background:var(--bg);border:1px solid var(--bd);border-radius:12px;overflow:hidden">'
    +'<div style="padding:9px 14px;background:var(--s1);border-bottom:1px solid var(--bd);display:flex;align-items:center;justify-content:space-between">'
    +'<div style="font-size:13px;font-weight:800;color:var(--tx)">🌐 시장 현황</div>'
    +'<div style="display:flex;align-items:center;gap:8px">'
    +'<div style="font-size:10px;color:#4b5563">갱신 '+ts+'</div>'
    +'<button onclick="window._ctRefreshMarket()" style="padding:3px 8px;background:transparent;border:1px solid var(--bd);border-radius:6px;color:var(--mt);cursor:pointer;font-size:10px">↺ 새로고침</button>'
    +'</div></div>'
...
window._ctRefreshMarket = async function(){
  _mktTs=0; // 캐시 무효화
  var el=document.getElementById('ct-market-card');
  if(!el) return;
  el.innerHTML='<div style="padding:10px;text-align:center;color:var(--mt);font-size:12px">🌐 시장 데이터 갱신 중...</div>';
  var mkt=await fetchMarket();
  el.innerHTML=buildMarketCard(mkt)||'<div style="padding:10px;color:#6b7280;font-size:12px">시장 데이터 불러오기 실패</div>';
};
...
function _alertGetList(){ return JSON.parse(localStorage.getItem('ct_alerts')||'[]'); }
function _alertSaveList(arr){ localStorage.setItem('ct_alerts', JSON.stringify(arr)); }

window._ctAddAlert = function(){
  var sym  = (document.getElementById('ct-alert-sym')||{}).value||'';
  var price = parseFloat((document.getElementById('ct-alert-price')||{}).value)||0;
  var dir  = (document.getElementById('ct-alert-dir')||{}).value||'above';
  var note = (document.getElementById('ct-alert-note')||{}).value||'';
  if(!sym||!price){ alert('종목과 목표가를 입력하세요'); return; }
  var list=_alertGetList();
  list.push({id:Date.now(), sym:sym.toUpperCase(), price:price, dir:dir, note:note, active:true, ts:Date.now()});
  _alertSaveList(list);
  _ctRenderAlerts();
  _ctStartAlertPolling();
  var inp=document.getElementById('ct-alert-price'); if(inp) inp.value='';
};

window._ctDeleteAlert = function(id){
  _alertSaveList(_alertGetList().filter(function(a){return a.id!==id;}));
  _ctRenderAlerts();
};

function _ctRenderAlerts(){
  var el=document.getElementById('ct-alert-list'); if(!el) return;
  var list=_alertGetList().filter(function(a){return a.active;});
  if(!list.length){ el.innerHTML='<div style="font-size:12px;color:#6b7280;text-align:center;padding:10px">설정된 알림 없음</div>'; return; }
  el.innerHTML=list.map(function(a){
    var dC=a.dir==='above'?'#22c55e':'#ef4444';
    var dL=a.dir==='above'?'▲ 이상':'▼ 이하';
    return '<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.04)">'
      +'<div style="flex:1">'
      +'<span style="font-size:13px;font-weight:700;color:var(--ac)">'+a.sym+'</span>'
      +'<span style="margin-left:8px;font-size:12px;color:'+dC+'">'+dL+' <b>'+a.price.toLocaleString()+'</b></span>'
      +(a.note?'<span style="font-size:11px;color:#6b7280;margin-left:6px">'+a.note+'</span>':'')
      +'</div>'
      +'<button onclick="window._ctDeleteAlert('+a.id+')" style="background:none;border:none;color:#ef4444;cursor:pointer;font-size:14px;padding:2px">🗑</button>'
      +'</div>';
  }).join('');
}

function _ctStartAlertPolling(){
  if(_alertInterval) return;
  _alertInterval = setInterval(async function(){
    var list=_alertGetList().filter(function(a){return a.active;});
    if(!list.length){ clearInterval(_alertInterval); _alertInterval=null; return; }
    for(var i=0;i<list.length;i++){
      var a=list[i]; if(!a.active) continue;
      try {
        var sym=/^\d{4,6}$/.test(a.sym)?'KRX:'+a.sym:a.sym;
        ...
        var r=d&&d.chart&&d.chart.result&&d.chart.result[0];
        if(!r) continue;
        var cur=r.meta.regularMarketPrice;
        var triggered=(a.dir==='above'&&cur>=a.price)||(a.dir==='below'&&cur<=a.price);
        if(triggered){
          // 브라우저 알림
          if(Notification&&Notification.permission==='granted'){
            new Notification('⚠ 가격 알림: '+a.sym, {body:a.sym+' 현재가 '+cur.toLocaleString()+' → '+(a.dir==='above'?'목표가':'손절가')+' '+a.price.toLocaleString()+' 도달!\n'+a.note, icon:'/favicon.ico'});
          }
          // 탭 알림
          var orig=document.title;
          var blink=setInterval(function(){document.title=document.title==='⚠ 알림!'?orig:'⚠ 알림!';},800);
          setTimeout(function(){clearInterval(blink);document.title=orig;},10000);
          // 비활성화
          var fullList=_alertGetList(); var idx=fullList.findIndex(function(x){return x.id===a.id;}); if(idx>=0){ fullList[idx].active=false; _alertSaveList(fullList); }
          _ctRenderAlerts();
        }
      } catch(_){}
    }
...
window._ctRequestNotifPerm = function(){
  if(!Notification){ alert('이 브라우저는 알림을 지원하지 않습니다.'); return; }
  Notification.requestPermission().then(function(p){
    var btn=document.getElementById('ct-notif-btn');
    if(btn) btn.textContent=p==='granted'?'✅ 알림 허용됨':'❌ 알림 거부됨';
  });
};
...
// ── 외국인/기관 수급 (한국 주식) ──
// ══════════════════════════════════════
async function fetchKrSupply(code){
  // 네이버 금융 투자자별 거래 현황
  ...
  try {
    ...
    for(var pi=0;pi<proxies.length;pi++){
      ...
      var url='https://finance.naver.com/item/frgn.naver?code='+code;
      try {
        var resp=await fetch(proxies[pi]+encodeURIComponent(url),{signal:ctrl.signal});
        if(!resp||!resp.ok) continue;
        var html=await resp.text();
        if(proxies[pi].includes('allorigins')){ try{ var j=JSON.parse(html); html=j.contents||html; }catch(_){} }
        // 외국인 보유비율
        var frgnMatch=html.match(/외국인보유[\s\S]*?([\d,]+\.?\d*)\s*%/);
        var frgnNetMatch=html.match(/외국인[\s\S]*?([+-]?[\d,]+)\s*<\/td>/);
        if(frgnMatch||frgnNetMatch){
          return {
            frgnRatio: frgnMatch?parseFloat(frgnMatch[1].replace(/,/g,'')):null,
            frgnNet: frgnNetMatch?parseInt(frgnNetMatch[1].replace(/[,+]/g,'')):null,
          };
        }
      } catch(_){ continue; }
    }
...
function buildTechSummaryCard(ind, cur, fp){
  ...
  h+='</div>';
  // 핵심 지표 수치 요약 행
  var bbPct = bb&&cur&&(bb.upper-bb.lower)>0 ? Math.round((cur-bb.lower)/(bb.upper-bb.lower)*100) : null;
  var bbPctC = bbPct!==null?(bbPct>80?'#ef4444':bbPct<20?'#22c55e':'#6b7280'):'#6b7280';
  var volR  = ind.vol&&ind.vol.avg>0 ? Math.round(ind.vol.cur/ind.vol.avg*10)/10 : null;
  var volRC = volR!==null?(volR>=1.5?'#22c55e':volR<0.7?'#ef4444':'#6b7280'):'#6b7280';
  var badges=[
    rsi!=null?{label:'RSI',val:Math.round(rsi),c:rsi<30?'#22c55e':rsi>70?'#ef4444':rsi<50?'#4ade80':'#f59e0b'}:null,
    mac?{label:'MACD히스토',val:(mac.hist>0?'+':'')+( mac.hist?mac.hist.toFixed(2):'0'),c:mac.hist>0?'#22c55e':'#ef4444'}:null,
    bbPct!==null?{label:'BB%B',val:bbPct+'%',c:bbPctC}:null,
    volR!==null?{label:'거래량비율',val:volR+'x',c:volRC}:null,
    ind.rsiDiv?{label:'RSI다이버',val:ind.rsiDiv==='bullish'?'강세▲':'약세▼',c:ind.rsiDiv==='bullish'?'#22c55e':'#ef4444'}:null,
    ind.macdCross?{label:'MACD크로스',val:ind.macdCross==='golden'?'골든✓':'데드✗',c:ind.macdCross==='golden'?'#22c55e':'#ef4444'}:null,
  ].filter(Boolean);
  if(badges.length){
    h+='<div style="display:flex;flex-wrap:wrap;gap:5px;padding:10px 14px;border-top:1px solid rgba(255,255,255,.04)">';
    badges.forEach(function(b){
      h+='<div style="padding:4px 9px;background:'+b.c+'18;border:1px solid '+b.c+'44;border-radius:20px;font-size:11px;color:'+b.c+';font-weight:700">'
        +b.label+' <b>'+b.val+'</b></div>';
    });
    h+='</div>';
  }
  h+='</div>';
...
function buildTabContent(){ return buildChartPane()+buildAnalyzePane()+buildHistoryPane()+buildPortfolioPane()+buildWatchlistPane()+buildToolsPane()+buildScreenerPane()+buildCalendarPane()+buildAlertsPane()+buildTheoryPane(); }
...
function buildHistoryPane(){
  ...
  +'<th>날짜</th><th>종목</th><th>진입가</th><th>청산가</th><th>손익%</th><th>결과</th><th></th>'
  +'</tr></thead>'
...
// ── 매매 일지 패턴 분석 ──
// ── 월간 성과 리뷰 ──
function buildMonthlyReview(hist){
  if(!hist||hist.length<2) return '';
  var byMonth={};
  hist.forEach(function(h){
    var k=(h.date||'').slice(0,7); if(!k) return;
    if(!byMonth[k]) byMonth[k]={total:0,win:0,lose:0,hold:0,pnls:[],syms:[]};
    byMonth[k].total++;
    byMonth[k].syms.push(h.sym);
    if(h.result==='성공') byMonth[k].win++;
    else if(h.result==='실패') byMonth[k].lose++;
    else if(h.result==='보유중') byMonth[k].hold++;
    if(h.exitPrice&&h.price) byMonth[k].pnls.push((h.exitPrice-h.price)/h.price*100);
  });
  var months=Object.keys(byMonth).sort().slice(-6);
  if(months.length<1) return '';

  return '<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:14px">'
  +'<div style="font-size:14px;font-weight:800;color:var(--tx);margin-bottom:12px">📅 월간 성과 리뷰 (최근 6개월)</div>'
  +'<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px">'
  +'<thead><tr style="border-bottom:1px solid var(--bd)">'
  +'<th style="padding:6px 10px;text-align:left;color:#6b7280;font-weight:600">월</th>'
  +'<th style="padding:6px;text-align:center;color:#6b7280;font-weight:600">분석수</th>'
  +'<th style="padding:6px;text-align:center;color:#22c55e;font-weight:600">성공</th>'
  +'<th style="padding:6px;text-align:center;color:#ef4444;font-weight:600">실패</th>'
  +'<th style="padding:6px;text-align:center;color:#6b7280;font-weight:600">승률</th>'
  +'<th style="padding:6px;text-align:center;color:#f59e0b;font-weight:600">평균 손익</th>'
  +'</tr></thead><tbody>'
  +months.map(function(m){
    var d=byMonth[m];
    var done=d.win+d.lose;
    var wr=done?Math.round(d.win/done*100):null;
    var avgPnl=d.pnls.length?Math.round(d.pnls.reduce(function(a,b){return a+b;},0)/d.pnls.length*10)/10:null;
    var wrC=wr===null?'#6b7280':wr>=60?'#22c55e':wr>=40?'#f59e0b':'#ef4444';
    var pnlC=avgPnl===null?'#6b7280':avgPnl>=0?'#22c55e':'#ef4444';
    return '<tr style="border-bottom:1px solid rgba(255,255,255,.04)">'
      +'<td style="padding:7px 10px;color:var(--tx);font-weight:700">'+m+'</td>'
      +'<td style="padding:7px;text-align:center;color:var(--mt)">'+d.total+'</td>'
      +'<td style="padding:7px;text-align:center;color:#22c55e">'+d.win+'</td>'
      +'<td style="padding:7px;text-align:center;color:#ef4444">'+d.lose+'</td>'
      +'<td style="padding:7px;text-align:center;color:'+wrC+';font-weight:800">'+(wr!==null?wr+'%':'—')+'</td>'
      +'<td style="padding:7px;text-align:center;color:'+pnlC+';font-weight:800">'+(avgPnl!==null?(avgPnl>0?'+':'')+avgPnl+'%':'—')+'</td>'
      +'</tr>';
  }).join('')
  +'</tbody></table></div></div>';
}
...
// ── 알림 탭 ──
// ══════════════════════════════════════
function buildAlertsPane(){
  return '<div class="ct-pane" data-pane="alerts" style="display:none;padding:16px">'

  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:14px">'
  +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">'
  +'<div style="font-size:15px;font-weight:800;color:var(--tx)">⏰ 가격 알림 설정</div>'
  +'<button id="ct-notif-btn" onclick="window._ctRequestNotifPerm()" style="padding:5px 12px;background:rgba(59,130,246,.15);border:1px solid #3b82f6;border-radius:8px;color:#60a5fa;font-size:11px;font-weight:700;cursor:pointer">'
  +(typeof Notification!=='undefined'&&Notification.permission==='granted'?'✅ 알림 허용됨':'🔔 알림 허용')
  +'</button>'
  +'</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">종목/티커</div>'
  +'<input id="ct-alert-sym" class="ct-input" placeholder="005930, AAPL..." oninput="window._ctSearchInput&&window._ctSearchInput(this.value)"></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">목표 가격</div>'
  +'<input id="ct-alert-price" class="ct-input" type="number" placeholder="가격"></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">조건</div>'
  +'<select id="ct-alert-dir" class="ct-input">'
  +'<option value="above">▲ 이상 (돌파)</option>'
  +'<option value="below">▼ 이하 (이탈/손절)</option>'
  +'</select></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">메모</div>'
  +'<input id="ct-alert-note" class="ct-input" placeholder="목표가, 손절가..."></div>'
  +'</div>'
  +'<button class="ct-btn" onclick="window._ctAddAlert()">+ 알림 추가</button>'
  +'<div style="margin-top:4px;font-size:11px;color:#4b5563">⚠ 이 탭이 열려 있는 동안 1분 간격으로 가격을 확인합니다. 브라우저 알림을 허용해야 알림이 표시됩니다.</div>'
  +'</div>'

  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px">'
  +'<div style="font-size:14px;font-weight:800;color:var(--tx);margin-bottom:10px">📋 활성 알림</div>'
  +'<div id="ct-alert-list"><div style="font-size:12px;color:#6b7280;text-align:center;padding:10px">설정된 알림 없음</div></div>'
  +'</div>'
...
// ── 멀티 종목 스크리너 ──
// ══════════════════════════════════════
function buildScreenerPane(){
  var presets = [
    {label:'국내 대형주', tickers:'005930,000660,035420,035720,005380,000270,005490,051910,066570,373220'},
    {label:'반도체', tickers:'005930,000660,042700,045180,058470,096530,357780,403870'},
    {label:'미국 빅테크', tickers:'AAPL,MSFT,GOOGL,AMZN,NVDA,META,TSLA,AVGO'},
    {label:'미국 금융', tickers:'JPM,BAC,GS,MS,WFC,C,BRK-B'},
    {label:'ETF', tickers:'SPY,QQQ,IWM,GLD,TLT,VIX'},
  ];
  return '<div class="ct-pane" data-pane="screener" style="display:none;padding:16px">'
  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:14px">'
  +'<div style="font-size:15px;font-weight:800;color:var(--tx);margin-bottom:4px">🔍 멀티 종목 스크리너</div>'
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:12px">여러 종목을 한 번에 분석해 기술적 점수 순으로 정렬합니다 (최대 15종목)</div>'
  // 프리셋
  +'<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px">'
  +presets.map(function(p){
    return '<button onclick="document.getElementById(\'ct-scr-tickers\').value=\''+p.tickers+'\'" '
      +'style="padding:5px 12px;background:var(--s2);border:1px solid var(--bd);border-radius:20px;color:var(--mt);font-size:11px;cursor:pointer">'+p.label+'</button>';
  }).join('')
  +'</div>'
  +'<div style="margin-bottom:10px">'
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:4px">티커 입력 (쉼표 구분, 국내주식은 숫자코드)</div>'
  +'<textarea id="ct-scr-tickers" class="ct-input" rows="2" placeholder="005930, 000660, AAPL, TSLA, NVDA ..."></textarea>'
  +'</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px">'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">RSI 최대</div>'
  +'<input id="ct-scr-rsi-max" class="ct-input" type="number" value="65" placeholder="65"></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">최소 점수 (0~100)</div>'
  +'<input id="ct-scr-min-score" class="ct-input" type="number" value="50" placeholder="50"></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">정렬</div>'
  +'<select id="ct-scr-sort" class="ct-input">'
  +'<option value="score">점수 높은 순</option>'
  +'<option value="rsi">RSI 낮은 순</option>'
  +'<option value="vol">거래량 증가 순</option>'
  +'</select></div>'
  +'</div>'
  +'<button class="ct-btn" onclick="window._ctRunScreener()">🔍 스크리닝 실행</button>'
  +'</div>'
  +'<div id="ct-scr-result"></div>'
  +'</div>';
}

// ── 스크리너 실행 ──
window._ctRunScreener = async function(){
  var raw=(document.getElementById('ct-scr-tickers')||{}).value||'';
  var rsiMax=parseFloat((document.getElementById('ct-scr-rsi-max')||{}).value)||65;
  var minScore=parseFloat((document.getElementById('ct-scr-min-score')||{}).value)||50;
  var sort=(document.getElementById('ct-scr-sort')||{}).value||'score';
  var res=document.getElementById('ct-scr-result');
  if(!res) return;

  // 티커 파싱
  var tickers=raw.split(/[\n,，\s]+/).map(function(t){return t.trim().toUpperCase();}).filter(function(t){return t.length>=1&&t.length<=10;}).slice(0,15);
  if(!tickers.length){ res.innerHTML='<div style="padding:14px;color:#f59e0b">티커를 입력하세요</div>'; return; }

  res.innerHTML='<div style="padding:14px;text-align:center;color:var(--mt)">⏳ '+tickers.length+'개 종목 분석 중... (종목당 약 2초 소요)</div>';

  var results=[];
  for(var i=0;i<tickers.length;i++){
    var t=tickers[i];
    var sym=/^\d{4,6}$/.test(t)?'KRX:'+t:t;
    var ticker=toYahooTicker(sym);
    res.innerHTML='<div style="padding:14px;text-align:center;color:var(--mt)">⏳ ('+( i+1)+'/'+tickers.length+') '+t+' 분석 중...</div>';
    try {
      var qdata=null;
      // 한국 주식
      if(/^\d{4,6}$/.test(t)){
        qdata=await fetchNaver(t);
      }
      // 글로벌
      if(!qdata){
        var resp=await fetch('/api/quote?symbol='+encodeURIComponent(ticker));
        if(resp&&resp.ok){
          ...
          if(json&&json.chart&&json.chart.result&&json.chart.result[0]){
            ...
            var q=r.indicators.quote[0];
            qdata={closes:q.close,opens:q.open,highs:q.high,lows:q.low,vols:q.volume,curPrice:r.meta.regularMarketPrice,currency:r.meta.currency,name:r.meta.shortName||t};
          }
        }
      }
      if(!qdata||!qdata.closes||qdata.closes.length<10){ results.push({t:t,error:true}); continue; }

      var closes=qdata.closes.filter(Boolean);
      var aData=computeAutoAnalysis(closes,qdata.opens||[],qdata.highs||[],qdata.lows||[],qdata.vols||[],qdata.curPrice);
      var qs=computeQuantStats(closes);
      var score=_scrScore(aData);
      var ind=aData.indicators||{};
      var rsi=ind.rsi;
      var volRatio=ind.vol&&ind.vol.avg>0?Math.round(ind.vol.cur/ind.vol.avg*10)/10:1;
      results.push({t:t,name:qdata.name||t,score:score,rsi:rsi,ind:ind,qs:qs,cur:qdata.currency,price:qdata.curPrice,sym:sym,structure:aData.structure,volRatio:volRatio});
    } catch(e){ results.push({t:t,error:true,errMsg:e.message}); }
  }

  // 필터 + 정렬
  var valid=results.filter(function(r){return !r.error&&r.score>=minScore;});
  if(sort==='score') valid.sort(function(a,b){return b.score-a.score;});
  else if(sort==='rsi') valid.sort(function(a,b){return (a.rsi||100)-(b.rsi||100);});
  else if(sort==='vol'){
    valid.sort(function(a,b){ return (b.volRatio||1)-(a.volRatio||1); });
  }
  var failed=results.filter(function(r){return r.error;});

  if(!valid.length&&!failed.length){ res.innerHTML='<div style="padding:14px;color:#6b7280">조건에 맞는 종목이 없습니다. 최소 점수를 낮춰보세요.</div>'; return; }

  var html='<div style="overflow-x:auto">'
    +'<table class="ct-hist-table"><thead><tr>'
    +'<th>종목</th><th>현재가</th><th>점수</th><th>RSI</th><th>MACD</th><th>MA</th><th>볼린저</th><th>샤프</th><th>MDD</th><th>판단</th><th></th>'
    +'</tr></thead><tbody>'
    +valid.map(function(r){
      var scoreC=r.score>=75?'#22c55e':r.score>=50?'#f59e0b':'#ef4444';
      var rsiC=!r.rsi?'#6b7280':r.rsi<30?'#22c55e':r.rsi<50?'#4ade80':r.rsi<70?'#f59e0b':'#ef4444';
      var ind=r.ind||{};
      var macd=ind.macd?ind.macd.hist>0?'▲':'▼':'—';
      var macdC=ind.macd?ind.macd.hist>0?'#22c55e':'#ef4444':'#6b7280';
      var ma=ind.sma&&ind.sma.s20&&ind.sma.s60?(ind.sma.s20>ind.sma.s60?'정배열':'역배열'):'—';
      var maC=ind.sma&&ind.sma.s20&&ind.sma.s60?(ind.sma.s20>ind.sma.s60?'#22c55e':'#ef4444'):'#6b7280';
      var bb=ind.bb&&r.price?(r.price<=ind.bb.lower*1.03?'하단↑':r.price>=ind.bb.upper*0.97?'상단↓':'중립'):'—';
      var bbC=ind.bb&&r.price?(r.price<=ind.bb.lower*1.03?'#22c55e':r.price>=ind.bb.upper*0.97?'#ef4444':'#6b7280'):'#6b7280';
      var fp3=function(v){ if(!v) return '—'; return r.cur==='USD'?'$'+v.toFixed(2):Math.round(v).toLocaleString(); };
      return '<tr>'
        +'<td><b style="color:var(--ac);cursor:pointer" onclick="window._ctSelectSym(\''+r.t+'\');window._ctSwitchTab(\'chart\')">'+r.t+'</b><br><span style="font-size:10px;color:#6b7280">'+r.name+'</span></td>'
        +'<td>'+fp3(r.price)+'</td>'
        +'<td><span style="font-weight:900;color:'+scoreC+'">'+r.score+'</span></td>'
        +'<td style="color:'+rsiC+'">'+(r.rsi||'—')+'</td>'
        +'<td style="color:'+macdC+'">'+macd+'</td>'
        +'<td style="color:'+maC+'">'+ma+'</td>'
        +'<td style="color:'+bbC+'">'+bb+'</td>'
        +'<td style="color:'+(r.qs&&r.qs.sharpe>=1?'#22c55e':r.qs&&r.qs.sharpe>=0?'#f59e0b':'#ef4444')+'">'+(r.qs?r.qs.sharpe:'—')+'</td>'
        +'<td style="color:#ef4444">'+(r.qs?'-'+r.qs.mdd+'%':'—')+'</td>'
        +'<td style="font-size:11px;color:var(--mt)">'+(r.structure?({box:'박스','trend-up':'상승↑','trend-down':'하락↓'}[r.structure]||r.structure):'—')+'</td>'
        +'<td><button onclick="window._ctSelectSym(\''+r.t+'\');window._ctSwitchTab(\'chart\')" style="padding:4px 8px;background:var(--ac);color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:11px">분석</button></td>'
        +'</tr>';
    }).join('')
    +(failed.length?'<tr><td colspan="11" style="font-size:11px;color:#4b5563;padding:8px 12px">⚠ 데이터 없음: '+failed.map(function(r){return r.t;}).join(', ')+'</td></tr>':'')
    +'</tbody></table></div>';
  res.innerHTML=html;
};

function _scrScore(aData){
  var score=50, ind=aData.indicators||{};
  var s=ind.sma||{};
  if(s.s20&&s.s60){ score+=s.s20>s.s60?10:-10; }
  if(s.s5&&s.s20){ score+=s.s5>s.s20?5:-5; }
  var rsi=ind.rsi;
  if(rsi!==null&&rsi!==undefined){
    if(rsi<30) score+=15;
  ...
  if(ind.rsiDiv==='bullish') score+=15;
  if(ind.rsiDiv==='bearish') score-=15;
  if(ind.macdCross==='golden') score+=15;
  if(ind.macdCross==='dead') score-=15;
  else if(ind.macd){ score+=ind.macd.hist>0?7:-7; }
  if(ind.bb&&aData.currentPrice){ if(aData.currentPrice<=ind.bb.lower*1.03) score+=10; else if(aData.currentPrice>=ind.bb.upper*0.97) score-=8; }
  if(ind.obvTrend==='up') score+=8;
  if(ind.obvTrend==='down') score-=8;
  var pat=ind.patterns||[];
  pat.forEach(function(p){ score+=p.type==='bullish'?8:p.type==='bearish'?-8:0; });
...
// ── 감시 목록 ──
// ══════════════════════════════════════
function buildWatchlistPane(){
  return '<div class="ct-pane" data-pane="watchlist" style="display:none;padding:16px">'
  +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">'
  +'<div><div style="font-size:15px;font-weight:800;color:var(--tx)">👁 감시 목록</div>'
  +'<div style="font-size:11px;color:#6b7280;margin-top:2px">목표가·손절가 설정 후 현재가 대비 거리를 확인합니다</div></div>'
  +'<button onclick="window._ctRefreshWatchlist()" style="padding:7px 14px;background:var(--ac);color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700">↺ 현재가 갱신</button>'
  +'</div>'
  // 추가 폼
  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:12px;padding:14px;margin-bottom:14px">'
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr auto;gap:8px;align-items:end;flex-wrap:wrap">'
  +'<div><div style="font-size:10px;color:#6b7280;margin-bottom:3px">티커/코드</div>'
  +'<input id="ct-wl-sym" class="ct-input" placeholder="005930, AAPL..."></div>'
  +'<div><div style="font-size:10px;color:#22c55e;margin-bottom:3px">목표가 (익절)</div>'
  +'<input id="ct-wl-tgt" class="ct-input" type="number" placeholder="목표가"></div>'
  +'<div><div style="font-size:10px;color:#ef4444;margin-bottom:3px">손절가</div>'
  +'<input id="ct-wl-sl" class="ct-input" type="number" placeholder="손절가"></div>'
  +'<div><div style="font-size:10px;color:#6b7280;margin-bottom:3px">메모</div>'
  +'<input id="ct-wl-memo" class="ct-input" placeholder="이유, 조건..."></div>'
  +'<button onclick="window._ctWlAdd()" style="padding:9px 14px;background:var(--ac);color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;white-space:nowrap">+ 추가</button>'
  +'</div></div>'
  +'<div id="ct-wl-list"><div style="text-align:center;padding:20px;color:#6b7280;font-size:12px">감시 종목을 추가하세요</div></div>'
  +'</div>';
}

function _wlGet(){ return JSON.parse(localStorage.getItem('ct_wl')||'[]'); }
function _wlSave(arr){ localStorage.setItem('ct_wl', JSON.stringify(arr)); }

window._ctWlAdd = function(){
  var sym  = ((document.getElementById('ct-wl-sym')||{}).value||'').trim().toUpperCase();
  var tgt  = parseFloat((document.getElementById('ct-wl-tgt')||{}).value)||0;
  var sl   = parseFloat((document.getElementById('ct-wl-sl')||{}).value)||0;
  var memo = (document.getElementById('ct-wl-memo')||{}).value||'';
  if(!sym){ alert('티커/코드를 입력하세요'); return; }
  var list=_wlGet();
  if(list.some(function(w){return w.sym===sym;})){ alert(sym+' 은(는) 이미 감시 목록에 있습니다'); return; }
  list.push({sym:sym, tgt:tgt, sl:sl, memo:memo, addedAt:Date.now()});
  _wlSave(list);
  var el=document.getElementById('ct-wl-sym'); if(el) el.value='';
  window._ctRefreshWatchlist();
};
window._ctWlRemove = function(sym){
  _wlSave(_wlGet().filter(function(w){return w.sym!==sym;}));
  window._ctRefreshWatchlist();
};

window._ctRefreshWatchlist = async function(){
  var list=_wlGet();
  var el=document.getElementById('ct-wl-list'); if(!el) return;
  if(!list.length){ el.innerHTML='<div style="text-align:center;padding:20px;color:#6b7280;font-size:12px">감시 종목을 추가하세요</div>'; return; }

  el.innerHTML='<div style="padding:10px;text-align:center;color:var(--mt);font-size:12px">⏳ 현재가 조회 중...</div>';

  var results=[];
  for(var i=0;i<list.length;i++){
    var w=list[i];
    try {
      var sym=/^\d{4,6}$/.test(w.sym)?'KRX:'+w.sym:w.sym;
      var ticker=toYahooTicker(sym);
      var resp=await fetch('/api/quote?symbol='+encodeURIComponent(ticker));
      if(!resp||!resp.ok){ results.push(Object.assign({},w,{cur:null})); continue; }
      var d=await resp.json();
      var r=d&&d.chart&&d.chart.result&&d.chart.result[0];
      var cur=r?r.meta.regularMarketPrice:null;
      var currency=r?r.meta.currency:'KRW';

      // 52주 고저가
      var q=r&&r.indicators&&r.indicators.quote&&r.indicators.quote[0];
      var allHi=q&&q.high?q.high.filter(Boolean):[];
      var allLo=q&&q.low?q.low.filter(Boolean):[];
      var w52hi=allHi.length?Math.max.apply(null,allHi):null;
      var w52lo=allLo.length?Math.min.apply(null,allLo):null;

      results.push(Object.assign({},w,{cur:cur, currency:currency, w52hi:w52hi, w52lo:w52lo}));
    } catch(_){ results.push(Object.assign({},w,{cur:null})); }
  }

  el.innerHTML='<div style="display:flex;flex-direction:column;gap:10px">'
    +results.map(function(r){
      var fp3=function(v){ if(!v) return '—'; return r.currency==='USD'?'$'+v.toFixed(2):Math.round(v).toLocaleString()+'원'; };
      var tgtPct=r.cur&&r.tgt?Math.round((r.tgt-r.cur)/r.cur*1000)/10:null;
      var slPct=r.cur&&r.sl?Math.round((r.sl-r.cur)/r.cur*1000)/10:null;
      // 52주 위치 바
      var posW=r.cur&&r.w52hi&&r.w52lo&&r.w52hi>r.w52lo?Math.round((r.cur-r.w52lo)/(r.w52hi-r.w52lo)*100):null;
      var posC=posW!==null?(posW>=80?'#ef4444':posW>=50?'#f59e0b':'#22c55e'):'#6b7280';

      return '<div style="background:var(--s2);border-radius:12px;padding:14px;border:1px solid var(--bd)">'
      +'<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:10px">'
      +'<div style="display:flex;align-items:center;gap:8px">'
      +'<b style="font-size:15px;color:var(--ac);cursor:pointer" onclick="window._ctSelectSym(\''+r.sym+'\');window._ctSwitchTab(\'chart\')">'+r.sym+'</b>'
      +(r.memo?'<span style="font-size:11px;color:#6b7280">'+r.memo+'</span>':'')
      +'</div>'
      +'<div style="display:flex;align-items:center;gap:8px">'
      +'<div style="font-size:20px;font-weight:900;color:var(--tx)">'+(r.cur?fp3(r.cur):'조회 실패')+'</div>'
      +'<button onclick="window._ctWlRemove(\''+r.sym+'\')" style="background:none;border:none;color:#4b5563;cursor:pointer;font-size:14px">🗑</button>'
      +'</div></div>'
      // 목표가/손절가 거리
      +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">'
      +(r.tgt?'<div style="padding:8px;background:rgba(34,197,94,.08);border-radius:8px;border:1px solid rgba(34,197,94,.2)">'
        +'<div style="font-size:10px;color:#22c55e">🎯 목표가</div>'
        +'<div style="font-size:14px;font-weight:800;color:#22c55e">'+fp3(r.tgt)+'</div>'
        +(tgtPct!==null?'<div style="font-size:11px;color:#22c55e">'+(tgtPct>0?'+':'')+tgtPct+'%</div>':'')
      +'</div>':'')
      +(r.sl?'<div style="padding:8px;background:rgba(239,68,68,.08);border-radius:8px;border:1px solid rgba(239,68,68,.2)">'
        +'<div style="font-size:10px;color:#ef4444">⛔ 손절가</div>'
        +'<div style="font-size:14px;font-weight:800;color:#ef4444">'+fp3(r.sl)+'</div>'
        +(slPct!==null?'<div style="font-size:11px;color:#ef4444">'+(slPct>0?'+':'')+slPct+'%</div>':'')
      +'</div>':'')
      +'</div>'
      // 52주 위치 바
      +(posW!==null?'<div>'
        +'<div style="display:flex;justify-content:space-between;font-size:10px;color:#6b7280;margin-bottom:3px">'
        +'<span>52주 저점 '+fp3(r.w52lo)+'</span>'
        +'<span style="color:'+posC+';font-weight:700">현재 위치 '+posW+'%</span>'
        +'<span>52주 고점 '+fp3(r.w52hi)+'</span>'
        +'</div>'
        +'<div style="height:6px;background:var(--s1);border-radius:3px;overflow:hidden">'
        +'<div style="height:100%;width:'+posW+'%;background:'+posC+';border-radius:3px"></div>'
        +'</div>'
      +'</div>':'')
      +'</div>';
    }).join('')
...
function buildToolsPane(){
  ...
  // ⑧ 배당 수익률 계산기
  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:16px">'
  +'<div style="font-size:15px;font-weight:800;color:var(--tx);margin-bottom:4px">💰 배당 수익률 계산기</div>'
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:12px">배당금과 주가로 수익률·복리 계산</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">💵 현재 주가</div>'
  +'<input id="ct-div-price" class="ct-input" type="number" placeholder="예: 60000" oninput="window._ctCalcDiv()"></div>'
  +'<div><div style="font-size:11px;color:#22c55e;margin-bottom:4px">💸 연간 배당금</div>'
  +'<input id="ct-div-annual" class="ct-input" type="number" placeholder="예: 1500" oninput="window._ctCalcDiv()"></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">📊 배당 횟수/년</div>'
  +'<select id="ct-div-freq" class="ct-input" onchange="window._ctCalcDiv()">'
  +'<option value="4">분기 (4회)</option><option value="2">반기 (2회)</option><option value="1">연 1회</option><option value="12">월배당 (12회)</option>'
  +'</select></div>'
  +'</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">💰 투자 금액</div>'
  +'<input id="ct-div-invest" class="ct-input" type="number" placeholder="예: 10000000" oninput="window._ctCalcDiv()"></div>'
  +'<div><div style="font-size:11px;color:#60a5fa;margin-bottom:4px">📈 연간 배당 성장률 (%)</div>'
  +'<input id="ct-div-growth" class="ct-input" type="number" value="5" placeholder="5" oninput="window._ctCalcDiv()"></div>'
  +'</div>'
  +'<div id="ct-div-result"><div style="font-size:12px;color:#6b7280">주가와 배당금을 입력하세요</div></div>'
  +'</div>'

  // ⑨ 상관계수 계산기
  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:16px">'
  +'<div style="font-size:15px;font-weight:800;color:var(--tx);margin-bottom:4px">📐 두 자산 상관계수</div>'
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:12px">두 종목의 가격 데이터를 조회해 상관관계를 계산합니다 (+1=완전 동행, -1=완전 역행)</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:end;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">자산 A</div>'
  +'<input id="ct-corr-a" class="ct-input" placeholder="예: 005930, SPY" value="SPY"></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">자산 B</div>'
  +'<input id="ct-corr-b" class="ct-input" placeholder="예: QQQ, 000660" value="QQQ"></div>'
  +'<button onclick="window._ctCalcCorr()" style="padding:9px 14px;background:var(--ac);color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;white-space:nowrap">계산</button>'
  +'</div>'
  +'<div id="ct-corr-result"><div style="font-size:12px;color:#6b7280">두 자산 티커를 입력하고 계산 버튼을 누르세요</div></div>'
  +'</div>'
...
// ── 배당 계산기 로직 ──
window._ctCalcDiv = function(){
  var price  = parseFloat((document.getElementById('ct-div-price')||{}).value)||0;
  var annual = parseFloat((document.getElementById('ct-div-annual')||{}).value)||0;
  var freq   = parseInt((document.getElementById('ct-div-freq')||{}).value)||4;
  var invest = parseFloat((document.getElementById('ct-div-invest')||{}).value)||0;
  var growth = parseFloat((document.getElementById('ct-div-growth')||{}).value)||5;
  var res    = document.getElementById('ct-div-result'); if(!res) return;
  if(!price||!annual){ res.innerHTML='<span style="color:#6b7280">주가와 연간 배당금을 입력하세요</span>'; return; }

  var yld=annual/price*100;
  var perPay=annual/freq;
  var sharesForM=invest>0?Math.floor(invest/price):0;
  var annualIncome=invest>0?sharesForM*annual:0;
  // 배당 성장 (10년)
  var yld10=yld*Math.pow(1+growth/100,10);
  // 복리 재투자 10년 (DRIP 시뮬)
  var shares10=sharesForM, divPrice=price;
  for(var y=0;y<10;y++){
    var yearDiv=shares10*annual;
    shares10+=Math.floor(yearDiv/divPrice);
    divPrice*=1.03; annual*=(1+growth/100);
  }
  var totalVal10=shares10*divPrice;

  res.innerHTML='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">'
  +'<div style="padding:10px;background:rgba(34,197,94,.08);border-radius:8px;border:1px solid rgba(34,197,94,.2)">'
  +'<div style="font-size:10px;color:#22c55e">현재 배당수익률</div>'
  +'<div style="font-size:22px;font-weight:900;color:#22c55e">'+yld.toFixed(2)+'%</div></div>'
  +'<div style="padding:10px;background:var(--s2);border-radius:8px">'
  +'<div style="font-size:10px;color:#6b7280">1회 배당금</div>'
  +'<div style="font-size:18px;font-weight:900;color:var(--tx)">'+Math.round(perPay).toLocaleString()+'</div>'
  +'<div style="font-size:10px;color:#6b7280">주당 ('+freq+'회/년)</div></div>'
  +'<div style="padding:10px;background:rgba(96,165,250,.08);border-radius:8px">'
  +'<div style="font-size:10px;color:#60a5fa">10년 후 배당수익률</div>'
  +'<div style="font-size:18px;font-weight:900;color:#60a5fa">'+yld10.toFixed(2)+'%</div>'
  +'<div style="font-size:10px;color:#6b7280">성장률 '+growth+'% 가정</div></div>'
  +'</div>'
  +(invest?'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px">'
  +'<div style="padding:10px;background:var(--s2);border-radius:8px">'
  +'<div style="font-size:10px;color:#6b7280">매수 수량 ('+invest.toLocaleString()+'원 기준)</div>'
  +'<div style="font-size:18px;font-weight:800;color:var(--tx)">'+sharesForM.toLocaleString()+'주</div>'
  +'<div style="font-size:11px;color:#22c55e">연간 수령 '+Math.round(annualIncome).toLocaleString()+'원</div></div>'
  +'<div style="padding:10px;background:rgba(245,158,11,.08);border-radius:8px">'
  +'<div style="font-size:10px;color:#f59e0b">10년 복리 재투자 후 평가금액</div>'
  +'<div style="font-size:18px;font-weight:800;color:#f59e0b">'+Math.round(totalVal10).toLocaleString()+'원</div>'
  +'<div style="font-size:10px;color:#6b7280">(DRIP, 연'+growth+'% 성장 가정)</div></div>'
  +'</div>':'');
};

// ── 상관계수 계산기 ──
window._ctCalcCorr = async function(){
  var symA=((document.getElementById('ct-corr-a')||{}).value||'SPY').trim().toUpperCase();
  var symB=((document.getElementById('ct-corr-b')||{}).value||'QQQ').trim().toUpperCase();
  var res=document.getElementById('ct-corr-result'); if(!res) return;
  res.innerHTML='<div style="color:var(--mt);font-size:12px">⏳ 데이터 조회 중...</div>';

  try {
    var [dA, dB]=await Promise.all([
      fetch('/api/quote?symbol='+encodeURIComponent(toYahooTicker(/^\d{4,6}$/.test(symA)?'KRX:'+symA:symA))).then(function(r){return r.json();}),
      fetch('/api/quote?symbol='+encodeURIComponent(toYahooTicker(/^\d{4,6}$/.test(symB)?'KRX:'+symB:symB))).then(function(r){return r.json();})
    ]);
    var getCloses=function(d){
      var r=d&&d.chart&&d.chart.result&&d.chart.result[0]; if(!r) return [];
      return (r.indicators.quote[0].close||[]).filter(Boolean);
    };
    var cA=getCloses(dA), cB=getCloses(dB);
    var n=Math.min(cA.length, cB.length, 60);
    if(n<10){ res.innerHTML='<div style="color:#ef4444;font-size:12px">데이터 부족 — 다른 티커를 입력하세요</div>'; return; }
    cA=cA.slice(-n); cB=cB.slice(-n);
    // 수익률
    var rA=[], rB=[];
    for(var i=1;i<n;i++){ rA.push((cA[i]-cA[i-1])/cA[i-1]); rB.push((cB[i]-cB[i-1])/cB[i-1]); }
    var mA=rA.reduce(function(s,x){return s+x;},0)/rA.length;
    var mB=rB.reduce(function(s,x){return s+x;},0)/rB.length;
    var cov=rA.reduce(function(s,x,i){return s+(x-mA)*(rB[i]-mB);},0)/rA.length;
    var sA=Math.sqrt(rA.reduce(function(s,x){return s+Math.pow(x-mA,2);},0)/rA.length);
    var sB=Math.sqrt(rB.reduce(function(s,x){return s+Math.pow(x-mB,2);},0)/rB.length);
    var corr=sA&&sB?cov/(sA*sB):0;
    corr=Math.round(corr*1000)/1000;

    var corrC=corr>=0.7?'#ef4444':corr<=-.7?'#22c55e':corr>=0.4?'#f97316':corr<=-0.4?'#4ade80':'#6b7280';
    var corrLabel=corr>=0.7?'강한 양의 상관 — 함께 움직임':corr>=0.4?'보통 양의 상관':corr<=-0.7?'강한 음의 상관 — 헤지 효과':corr<=-0.4?'보통 음의 상관':'약한 상관 — 독립적 움직임';
    var barPct=(corr+1)/2*100;

    res.innerHTML='<div style="padding:14px;background:var(--s2);border-radius:10px">'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">'
    +'<div style="font-size:14px;font-weight:700;color:var(--tx)">'+symA+' vs '+symB+'</div>'
    +'<div style="font-size:24px;font-weight:900;color:'+corrC+'">'+corr+'</div>'
    +'</div>'
    +'<div style="height:8px;background:var(--bg);border-radius:4px;overflow:hidden;margin-bottom:6px">'
    +'<div style="height:100%;width:'+barPct+'%;background:'+corrC+';border-radius:4px"></div></div>'
    +'<div style="font-size:12px;color:'+corrC+'">'+corrLabel+'</div>'
    +'<div style="font-size:10px;color:#4b5563;margin-top:6px">'+n+'일 일간 수익률 기준 | -1(완전역행) ~ 0(독립) ~ +1(완전동행)</div>'
    +'</div>';
  } catch(e){ res.innerHTML='<div style="color:#ef4444;font-size:12px">조회 실패: '+e.message+'</div>'; }
};

// ── 계산 도구 탭 ──
function buildToolsPane(){
  return '<div class="ct-pane" data-pane="tools" style="display:none;padding:16px">'
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:14px">각 도구는 독립적으로 사용할 수 있습니다.</div>'
  // 배당 계산기
  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:16px">'
  +'<div style="font-size:15px;font-weight:800;color:var(--tx);margin-bottom:4px">💰 배당 수익률 계산기</div>'
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:12px">배당금과 주가로 수익률·복리 계산</div>'
  ...
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">📐 1차 : 2차 비율</div>'
  +'<select id="ct-split-ratio" class="ct-input" onchange="window._ctCalcSplit()">'
  +'<option value="50:50">50:50 (균등)</option><option value="60:40" selected>60:40 (1차 우선)</option>'
  +'<option value="70:30">70:30 (1차 집중)</option><option value="40:60">40:60 (2차 적극)</option>'
  +'</select></div>'
  +'</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#22c55e;margin-bottom:4px">🟢 1차 진입가</div>'
  ...
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">✖ ATR 배수</div>'
  +'<select id="ct-atr-mult" class="ct-input" onchange="window._ctCalcATR()">'
  +'<option value="1">1x</option><option value="1.5" selected>1.5x</option><option value="2">2x</option><option value="3">3x</option>'
  +'</select></div>'
  +'</div>'
  +'<div id="ct-atr-result"><div style="font-size:12px;color:#6b7280">진입가와 ATR을 입력하세요</div></div>'
  +'</div>'
  ...
  +'<div style="background:var(--bg);border:1px solid var(--bd);border-radius:14px;padding:16px;margin-bottom:16px">'
  +'<div style="font-size:15px;font-weight:800;color:var(--tx);margin-bottom:4px">⚖️ 손익분기 & 목표 수익률</div>'
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:12px">수수료 포함 손익분기점 + 목표% 달성 주가</div>'
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">💵 평균 매수단가</div>'
  +'<input id="ct-be-entry" class="ct-input" type="number" placeholder="평균 단가" oninput="window._ctCalcBE()"></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">💸 수수료+세금 (%)</div>'
  +'<input id="ct-be-fee" class="ct-input" type="number" value="0.33" step="0.01" oninput="window._ctCalcBE()"></div>'
  +'<div><div style="font-size:11px;color:#60a5fa;margin-bottom:4px">🎯 목표 수익률 (%)</div>'
  +'<input id="ct-be-target" class="ct-input" type="number" value="10" oninput="window._ctCalcBE()"></div>'
  +'</div>'
  +'<div id="ct-be-result"><div style="font-size:12px;color:#6b7280">평균 단가를 입력하세요</div></div>'
  +'</div>'
  // 수익 목표 계획기
  ...
  +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#22c55e;margin-bottom:4px">✅ 예상 승률 (%)</div>'
  +'<input id="ct-goal-wr" class="ct-input" type="number" value="55" oninput="window._ctCalcGoal()"></div>'
  +'<div><div style="font-size:11px;color:#22c55e;margin-bottom:4px">📈 평균 수익 (%)</div>'
  +'<input id="ct-goal-avgwin" class="ct-input" type="number" value="8" oninput="window._ctCalcGoal()"></div>'
  +'<div><div style="font-size:11px;color:#ef4444;margin-bottom:4px">📉 평균 손실 (%)</div>'
  +'<input id="ct-goal-avgloss" class="ct-input" type="number" value="4" oninput="window._ctCalcGoal()"></div>'
  +'</div>'
  +'<div id="ct-goal-result"><div style="font-size:12px;color:#6b7280">현재 계좌와 목표 금액을 입력하세요</div></div>'
  +'</div>'
  ...
  +'<div style="font-size:11px;color:#6b7280;margin-bottom:12px">시장 현황 환율 연동 (미로드 시 근사 환율 사용)</div>'
  +'<div style="display:grid;grid-template-columns:1fr auto 1fr auto;gap:8px;align-items:end;margin-bottom:10px">'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">금액</div>'
  +'<input id="ct-fx-amt" class="ct-input" type="number" placeholder="예: 1000" oninput="window._ctCalcFX()"></div>'
  +'<div style="padding-bottom:8px"><select id="ct-fx-from" class="ct-input" style="width:80px" onchange="window._ctCalcFX()">'
  +'<option value="USD">USD</option><option value="KRW">KRW</option><option value="JPY">JPY</option><option value="EUR">EUR</option>'
  +'</select></div>'
  +'<div><div style="font-size:11px;color:#6b7280;margin-bottom:4px">→ 변환 통화</div>'
  +'<select id="ct-fx-to" class="ct-input" onchange="window._ctCalcFX()">'
  +'<option value="KRW">KRW</option><option value="USD">USD</option><option value="JPY">JPY</option><option value="EUR">EUR</option>'
  +'</select></div>'
  +'<div style="padding-bottom:8px"><button class="ct-btn" style="padding:9px 14px;margin:0" onclick="window._ctCalcFX()">변환</button></div>'
